...
class Virtualizor_Enduser_API {
	...
	function call($path, $post = array(), $cookies = array()){
		...
		if(!empty($this->is_admin)) {
			$key = $this->v_generateRandStr(8);
			$apikey = $this->v_make_apikey($key, $this->pass);
	 ...
	 */
	function v_make_apikey($key, $pass){
	 ...
	 */
	function v_generateRandStr($length){	
		$randstr = "";	
		for($i = 0; $i < $length; $i++){	
